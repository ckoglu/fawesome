...
function updateThemeIcon() {
  ...
  themeIcon.className = dark ? 'fa fa-sun-o' : 'fa fa-moon-o';
}
updateThemeIcon();
...
function showAlert(msg, type) {
  ...
  box.innerHTML = `
    <div class="alertContent">
      <div class="alertIcon"><i class="${msg}"></i></div>
      <div class="alertMessage">
        ${
            type === "copy" ? `<b>${msg}</b> kopyalandı` :
            type === "delete" ? `<b>${msg}</b> silindi` :
            msg
        }
    </div>
    </div>`;
...
function renderGrid(query = '') {
  ...
  const list = q ? ICONS.filter(n => n.includes(q)) : ICONS;
  grid.innerHTML = '';
  ...
  if (list.length === 0) {
    ...
    empty.innerHTML = `<i class="fa fa-search"></i><span>"${query}" için ikon bulunamadı</span>`;
    grid.appendChild(empty);
    return;
  }

  const frag = document.createDocumentFragment();
  list.forEach(name => frag.appendChild(createCard(name)));
...
searchInput.addEventListener('input', () => {
  ...
  debounce = setTimeout(() => renderGrid(searchInput.value), 100);
});
